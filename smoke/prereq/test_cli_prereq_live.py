...
from smoke.lib.child_process import cmd_companion_init, cmd_companion_serve
...
def test_companion_init_scaffolds_user_config(
    ...
    assert (tmp_path / ".config" / "companion" / ".env").is_file()


def test_companion_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
